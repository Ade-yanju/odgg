...
import { AuthSystem } from '@/lib/RegistrationSystem';

// ── Shared tokens (identical to register page) ───────────────────────────────
const CSS = `
...
export default function LoginPage() {
  ...
  async function handleSubmit(e) {
    ...
    setStatus('loading');
    try {
      const { role } = await authSystem.login(form.email, form.password);
  ...
  async function handleGoogle() {
    ...
    setStatus('loading');
    try {
      const { role } = await authSystem.loginWithGoogle();
      router.push(getRedirect(role));
    } catch (err) {
      setStatus('idle');
          ...
          <div className="testimonial">
            ...
            <div className="testimonial-author">
              ...
              <div>
                ...
                <div className="testimonial-role">200L, Lead City University</div>
              </div>
            </div>
          </div>
        </div>
